...
public:
	...
	MRotation2d(UTriMesh* mesh, Matrix<double>* coordinates, double angle, double centre_x, double centre_y, Matrix<int> nums_to_rotate)
	{
	...
	void setup(UTriMesh* mesh, Matrix<double>* coordinates, double angle, double centre_x, double centre_y, Matrix<int> nums_to_rotate)
	{
		cout << "MRotation2d: Initializing..\n";
		m = mesh;
		coords = coordinates;
		if(m->gnpoin() != coords->rows()) cout << "MRotation2d: setup(): Error!\n";
		theta = angle; cx = centre_x; cy = centre_y;
		nrot = nums_to_rotate;
	}
	...
	Matrix<double> rhsvect_rotate()
	{
		...
		for(int b = 0; b < m->gnbpoin(); b++)
		{
			for(int i = 0; i < nrot.msize(); i++)
			{
				...
				{
					rhs(b,0) = drx(coords->get(b,0)-cx, coords->get(b, 1)-cy);
					rhs(b,1) = dry(coords->get(b,0)-cx, coords->get(b, 1)-cy);
				}
			}
		}
