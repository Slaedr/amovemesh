...
public:
	...
	// boundary_flags is a npoin-by-1 vector, containing 1 if the corresponding row in all_points is a boundary point and zero otherwise.
	{
		...
		A.setup(nbpoin+ndim+1,nbpoin+ndim+1);
		...
		for(i = 0; i < ndim; i++)
		{
			b[i].setup(nbpoin+ndim+1,1);
			b[i].zeros();
			coeffs[i].setup(nbpoin+ndim+1,1);
		}
	...
	void assembleLHS()
	{
		...
		for(i = 0; i < nbpoin; i++)
		{
			A.set(i,i, (this->*rbf)(0.0));			// set diagonal element in row i
			//cout << "RBF value = " << (this->*rbf)(0.0) << endl;
			for(j = i+1; j < nbpoin; j++)		// traverse lower triangular matrix
			{
				dist = 0;
				for(int id = 0; id < ndim; id++)
					dist += (bpoints(i,id) - bpoints(j,id))*(bpoints(i,id) - bpoints(j,id));
				dist = sqrt(dist);
				temp = (this->*rbf)(dist);
				A.set(i,j, temp);
				A.set(j,i, temp);
	...
	void move_step()
	{
		...
		Matrix<double> xold(nbpoin+ndim+1,1);
		xold.zeros();
