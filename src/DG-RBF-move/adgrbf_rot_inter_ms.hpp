...
public:
	...
	void set_boundary_rotation(double rot_angle)
	{
		if(isallocralpha == false)
		{
			ralpha = new Matrix<double>[ndgelem];
	...
	void movemesh()
	{
		...
		for(int ipoin = 0; ipoin < ninpoin; ipoin++)
		{
			...
			elemno = dg.find_containing_triangle(points(ipoin,0), points(ipoin,1), dg.elems.size()/2);

			// store DG element and area coords in points
			points(ipoin,2) = elemno;
		}
		...
		while(num_steps > 0)
		{
			...
			{
				for(int j = 0; j < ndim; j++)
					testdgpoints(i,j) = dgpoints(i,j) + bmotion.get(i,j);
			}
			...
			set_boundary_rotation(angle);
			calcralpha();

			// calculate new positions of interior points by mapping them to deformed DG elements using RBFs calculated before
			...
			// move dgpoints and the DG itself
			...
			if(num_steps > 0 && reflag == true)
			{
				...
				numtri++;
				calcMtt();
				// re-calculate containing elements
				int elemno;
				for(int ipoin = 0; ipoin < ninpoin; ipoin++)
				{
					// first find containing DG element by "walking-through" the DG
