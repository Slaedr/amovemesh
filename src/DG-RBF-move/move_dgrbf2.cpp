...
int main()
{
	string confile = "move_dgrbf2.control";
	...
	Matrix<double> bca = rot.rhsvect_angles();
	Matrix<double> bcd = rot.rhsvect_rotate();

	// carry out DG mapping procedure
	DGRBF2 d(2, coord, flags, &bca, &bcd, centre, 2, suprad);
	d.generateDG();
	d.movemesh();
