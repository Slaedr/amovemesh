...
Sorts the points first using a bin-sort, and then adds points bin-by-bin.
Aditya Kashi
July 6, 2015
...
public:
	...
	{
		// add super triangle and calculate bins
		preprocess(20);
		...
		for(int ipoin = 0; ipoin < npoints; ipoin++)
		{
			...
			for(int ifa = 0; ifa < voidpoly.size(); ifa++)		// for each face in void polygon
			{
				...
				for(int jfa = 0; jfa < newfaces.size(); jfa++)
				{
					...
					{
						/*if(faces[newfaces[jfa]].elem[0] == -4)
						{
							faces[newfaces[jfa]].elem[0] = elems.size()-1;
							elems.back().surr[2] = faces[newfaces[jfa]].elem[1];

							if(elems[faces[newfaces[jfa]].elem[1]].surr[0] == -2)
								if( (elems[faces[newfaces[jfa]].elem[1]].p[1]==faces[newfaces[jfa]].p[0] && elems[faces[newfaces[jfa]].elem[1]].p[2]==faces[newfaces[jfa]].p[1]) || (elems[faces[newfaces[jfa]].elem[1]].p[1]==faces[newfaces[jfa]].p[1] && elems[faces[newfaces[jfa]].elem[1]].p[2]==faces[newfaces[jfa]].p[0]) )
									elems[faces[newfaces[jfa]].elem[1]].surr[0] = elems.size()-1;		// set the new element as an element surrounding the neighboring element

							if(elems[faces[newfaces[jfa]].elem[1]].surr[1] == -2)
								if( (elems[faces[newfaces[jfa]].elem[1]].p[0]==faces[newfaces[jfa]].p[0] && elems[faces[newfaces[jfa]].elem[1]].p[2]==faces[newfaces[jfa]].p[1]) || (elems[faces[newfaces[jfa]].elem[1]].p[0]==faces[newfaces[jfa]].p[1] && elems[faces[newfaces[jfa]].elem[1]].p[2]==faces[newfaces[jfa]].p[0]) )
									elems[faces[newfaces[jfa]].elem[1]].surr[1] = elems.size()-1;		// set the new element as an element surrounding the neighboring element

							if(elems[faces[newfaces[jfa]].elem[1]].surr[2] == -2)
								if( (elems[faces[newfaces[jfa]].elem[1]].p[1]==faces[newfaces[jfa]].p[0] && elems[faces[newfaces[jfa]].elem[1]].p[0]==faces[newfaces[jfa]].p[1]) || (elems[faces[newfaces[jfa]].elem[1]].p[1]==faces[newfaces[jfa]].p[1] && elems[faces[newfaces[jfa]].elem[1]].p[0]==faces[newfaces[jfa]].p[0]) )
									elems[faces[newfaces[jfa]].elem[1]].surr[2] = elems.size()-1;		// set the new element as an element surrounding the neighboring element
								// note that the new element is faces[newfaces[jfa]].elem[0], and so the neighboring one is faces[newfaces[jfa]].elem[1].
						}*/
	...
	UTriMesh getmesh()
	{
		//TODO: create mesh using calculated data and return it
	}
	   ...
	   Rather, we compare the corresponding area-ratio. If the sign of the area of the triangle created by the new point changes because of opposite orientation, so does the area of the triangle being checked. */
	{
		...
		while(1)
		{
			if(ielem < 0 || ielem >= elems.size()) { cout << "Delaunay2D:   !! Reached an element index that is out of bounds!!\n"; }
			super = elems[ielem];
