/* Mesh movement using Delaunay graph (DG) mapping technique of Liu, Qin and Xia, for rotation of boundary points
Aditya Kashi
July 14, 2015
*/
...
public:
	...
	double c_x;
	double c_y;
	Matrix<int> numstorotate;
	UTriMesh* m;
	MRotation2d r;
	Delaunay2D dg;
	Matrix<double> newcoords;

	DGmove(int num_dimn, Matrix<double>* coords, Matrix<int> bflags, UTriMesh* mesh, double angle, double centre_x, double centre_y, Matrix<int> nums_to_rotate)
	{
		...
		theta = angle;
		numstorotate = nums_to_rotate;
		c_x = centre_x;
		c_y = centre_y;
		m = mesh;
		r.setup(m, m->getcoords(), angle, centre_x, centre_y, nums_to_rotate);
		bmotion = r.rhsvect_rotate();
	...
	void movemesh()
	{
		...
		while(num_steps > 0)
		{
			...
			//testdgpoints.zeros();
			getcoords();
			r.setup(m, &newcoords,theta,c_x,c_y,numstorotate);
			bmot = r.rhsvect_rotate();				// get latest bmot
			...
			writeMeshToVtu("ymesh-step"+to_string(step)+".vtu", *m);
