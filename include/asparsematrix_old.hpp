...
public:
	...
	double get(int x, int y)
	{
		...
		if(sorted == false)
		{
			...
			#endif
			#pragma omp parallel for default(none) private(i) shared(row_ind,col_ind,nnz,x,y,index)
			for(i = 0; i < nnz; i++)
			{
				if(row_ind[i]==x)
				{
	...
	*/
	void multiply(Mat& x, Mat* a, char paralel = 'n')
	{
		...
		{
			int i;
			#pragma omp parallel for default(none) private(i,temp) shared(val,row_ind,col_ind,nnz,k,a,x) if(paralel=='y')
			for(i = 0; i < nnz; i++)
			{
	...
	NOTE: Make sure dimensions of x and y are same. */
	{
		...
		for(int k = 0; k < x->cols(); k++)		// for each column of x
		{
			...
			#endif
			int i;
			#pragma omp parallel for default(none) private(i) shared(val,row_ind,col_ind,nnz,k,rownum,p,x,y,num) reduction(+:ans)
			for(i = 0; i < nnz; i++)
			{
				if(row_ind[i] == rownum)
				{
...
public:
	...
	*/
	...
	void getCRSMatrix(SMatrixCRS& rmat) const
	{
		rmat.nnz = 0;
		for(int i = 0; i < nrows; i++)
			rmat.nnz += rsize[i];
...
class MatrixCRS_traditional : public SparseMatrix
{
	double* val;
	int* col_ind;
	int* row_ptr;
...
public:
	MatrixCRS_traditional(int num_rows, int num_cols) : SparseMatrix(num_rows, num_cols)
	{
