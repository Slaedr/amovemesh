...
inline double Point::get(int idim)
{
	#if DEBUGW==1
	if(idim >= dim) {
		std::cout << "! Point: (): Dimension " << idim << " does not exist!" << std::endl;
...
struct Face
{
	...
	int elem[3];		// elements on 3 side of the face
};
...
public:	
	...
	int surr[4];		///< Indices of surrounding tets.
...
public:
	...
	int find_containing_triangle(double xx, double yy, int startelement) const;
...
Walkdata Delaunay3D::find_containing_triangle_and_area_coords(double xx, double yy, int startelement)
